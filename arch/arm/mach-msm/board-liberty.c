...
//	printk(KERN_INFO "%s():\n", __func__);
	...
	gpio_request(LIBERTY_GPIO_VCM_PD, "cam_pwr_on");
	gpio_direction_output(LIBERTY_GPIO_VCM_PD, 0);
	gpio_free(LIBERTY_GPIO_VCM_PD);
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k4e1gx_data = {
	...
	.sensor_reset   = LIBERTY_GPIO_CAM_RST_N,
	.vcm_pwd        = LIBERTY_GPIO_VCM_PD,
...
static void __init liberty_map_io(void)
{
	...
	printk(KERN_INFO "Liberty_init L2-cache latency 3 cyc for 7x27\n");
#endif
}
