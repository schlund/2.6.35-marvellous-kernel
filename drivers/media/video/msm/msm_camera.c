...
int msm_camera_flash(struct msm_sync *sync, int level)
{
	...
	uint8_t phy_flash = 0;
	int ret = 0;
	const uint32_t focus_position_20cm = 300;
	...
	case MSM_CAMERA_LED_HIGH:
		...
		sync->sdata->flash_cfg->postpone_led_mode = focus_position > focus_position_20cm ? FL_MODE_FLASH_level2 : FL_MODE_FLASH_level1;
		phy_flash = 0;
		break;
