		...
		struct device_attribute *attr, char *buf)
{
	...
	if (i2c_himax_read(ts_data->client, himax_command, data, 64,
...
#ifdef ABS_MT_AMPLITUDE
			input_report_abs(ts->input_dev, ABS_MT_AMPLITUDE, 0);
#endif
#ifdef ABS_MT_POSITION
...
static int himax8526a_resume(struct i2c_client *client)
{
	...
	data[0] = 0x04;
	data[1] = 0x02;
	i2c_himax_write(ts->client, 0xDD, &data[0], 2, HIMAX_I2C_RETRY_TIMES);
