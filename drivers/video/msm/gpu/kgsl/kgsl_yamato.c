...
kgsl_yamato_init_pwrctrl(struct kgsl_device *device)
{
	...
	if (IS_ERR(device->pwrctrl.ebi1_clk)) {
		...
		int ret;
		ret = clk_set_rate(device->pwrctrl.ebi1_clk,
				device->pwrctrl.clk_freq[KGSL_AXI_HIGH] * 1000);
		if (ret < 0)
		KGSL_DRV_INFO("ebi1 clk_set_rate failed for freq %d. %d\n", device->pwrctrl.clk_freq[KGSL_AXI_HIGH] * 1000, ret);
		KGSL_DRV_INFO("kgsl_yamato_init_pwrctrl: clk_set_rate %d\n", device->pwrctrl.clk_freq[KGSL_AXI_HIGH] * 1000);
	}
...
static int kgsl_yamato_start(struct kgsl_device *device, unsigned int init_ram)
{
	...
	*/
	...
	if (device->chip_id != KGSL_CHIPID_LEIA_REV470) {
		...
		while (i) {
			kgsl_yamato_regwrite(device, REG_RBBM_PM_OVERRIDE1, 0xFDC001E0);
			kgsl_yamato_regread(device, REG_RBBM_PM_OVERRIDE1, &override1);
			kgsl_yamato_regwrite(device, REG_RBBM_PM_OVERRIDE2, 0x38);
			kgsl_yamato_regread(device, REG_RBBM_PM_OVERRIDE2, &override2);
			KGSL_DRV_INFO("Read OVERRIDE1 = 0x%x, OVERRIDE2 = 0x%x !!\n",
					override1, override2);
			if (((override1 & 0xFDC001E0) == 0xFDC001E0) &&
				((override2 & 0x00000038) == 0x00000038)) {
				KGSL_DRV_INFO("OVERRIDE1 OVERRIDE2 set properly\n");
				break;
			}
			i--;
		}
	...
	kgsl_yamato_regread(device, REG_RBBM_WAIT_IDLE_CLOCKS, &idle_clocks);
	KGSL_DRV_INFO("WAIT_IDLE_CLOCKS was 0x%08X\n", idle_clocks);
	idle_clocks = 0xFF;
	kgsl_yamato_regwrite(device, REG_RBBM_WAIT_IDLE_CLOCKS, idle_clocks);
	KGSL_DRV_INFO("WAIT_IDLE_CLOCKS is now 0x%08X\n", idle_clocks);

	kgsl_yamato_regread(device, REG_CP_DEBUG, &cp_debug);
	cp_debug |= (1 << 27);
	kgsl_yamato_regwrite(device, REG_CP_DEBUG, cp_debug);
	kgsl_yamato_regread(device, REG_CP_DEBUG, &cp_debug);
	KGSL_DRV_INFO("CP_DEBUG is now 0x%08X\n", cp_debug);

	KGSL_DRV_INFO("Timestamp: %08X wptr = %08X", yamato_device->ringbuffer.timestamp, yamato_device->ringbuffer.wptr);

	mod_timer(&device->idle_timer, jiffies + FIRST_TIMEOUT);
	status = KGSL_SUCCESS;
#ifdef CONFIG_KGSL_PER_PROCESS_PAGE_TABLE
	KGSL_DRV_INFO("msm_kgsl: initialized dev=%d mmu=%s "
...
{
	struct kgsl_yamato_device *yamato_device = KGSL_YAMATO_DEVICE(device);
	KGSL_DRV_INFO("kgsl_yamato_stop (device=%p) timestamp = %08X wptr = %08X\n", device, yamato_device->ringbuffer.timestamp, yamato_device->ringbuffer.wptr);
...
static unsigned int kgsl_yamato_isidle(struct kgsl_device *device)
{
	...
	if (rb->flags & KGSL_FLAGS_STARTED) {
		...
		GSL_RB_GET_READPTR(rb, &rb->rptr);
		ts_processed = device->ftbl.device_cmdstream_readtimestamp(device, KGSL_TIMESTAMP_RETIRED);
		if (!device->active_cnt && (rb->rptr == rb->wptr) && (rb->timestamp == ts_processed)) {
			/* Is the core idle? */
