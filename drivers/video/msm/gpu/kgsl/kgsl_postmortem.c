...
static int kgsl_dump_yamato(struct kgsl_device *device)
{
	...
	KGSL_LOG_DUMP("RBBM:   WAIT_IDLE_CLOCKS = %08X", r1);
...
#endif
	kgsl_regread(device, REG_CP_DEBUG, &r1);
	KGSL_LOG_DUMP("CP_DEBUG       = %08X\n", r1);
	...
	virt_imem_cfg = ioremap(0xA8500000, 12);        /* IMEM_CFG */
	if (virt_imem_cfg) {
		KGSL_LOG_DUMP("IMEM_CONFIG = %08X\n", readl(virt_imem_cfg));
		iounmap(virt_imem_cfg);
	} else
		KGSL_LOG_DUMP("ioremap of IMEM_CONFIG failed\n");
error_vfree:
	vfree(rb_copy);
end:
	return result;
