			...
			unsigned int flags)
{
	...
	KGSL_CTXT_INFO("from %p to %p flags %d timestamp %08X wptr %08X\n",
			yamato_device->drawctxt_active, drawctxt, flags, yamato_device->ringbuffer.timestamp, yamato_device->ringbuffer.wptr);
	/* save old context*/
	if (active_ctxt != NULL) {
	...
	if (drawctxt != NULL) {
		...
		KGSL_DRV_INFO("restore ctxt start timestamp %08X wptr = %08X", yamato_device->ringbuffer.timestamp, yamato_device->ringbuffer.wptr);
